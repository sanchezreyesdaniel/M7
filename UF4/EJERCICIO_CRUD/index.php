...
function deleteRow($tableName, $rowId, $pdo) {
    // Verificar si $rowId está definido y no está vacío
    if (!isset($rowId) || $rowId === '') {
        echo "<p style='font-family: Arial, sans-serif; margin-top: 20px; color: #f44336;'>Please provide a valid row ID.</p>";
        return;
    }
    ...
    try {
        ...
        echo "<p style='font-family: Arial, sans-serif; margin-top: 20px; color: #4CAF50;'>Row with ID $rowId deleted successfully.</p>";
    } catch (PDOException $e) {
        echo "<p style='font-family: Arial, sans-serif; margin-top: 20px; color: #f44336;'>Error deleting row: " . $e->getMessage() . "</p>";
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    } elseif (isset($_POST["deleteRow"])) { // Nueva condición para manejar el comando de eliminación
        $tableName = $_POST["customTableName"];
        $rowId = $_POST["customRowId"];
...
<body>
    <div class="container">
        ...
        <?php
        $idOrAll = isset($commandParts[2]) ? $commandParts[2] : null; // Definir $idOrAll antes del bloque condicional
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["userInput"])) {
            ...
            elseif ($action === 'delete' && $customTableName !== null && $idOrAll !== null) { // Agregar condición para el comando delete
                $rowId = $_POST["customRowId"]; // Obtener el ID de fila desde el formulario
                try {
                    $pdo = new PDO("pgsql:host=localhost;dbname=usuaris", "postgres", "root");
                    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                    deleteRow($customTableName, $rowId, $pdo); // Llamar a la función deleteRow con el ID de la fila
                } catch (PDOException $e) {
                    echo "<p style='font-family: Arial, sans-serif; margin-top: 20px; color: #f44336;'>Database connection error: " . $e->getMessage() . "</p>";
                }
            }
            ...
            echo '
                    <form class="mt-5" method="post">
                        ...
                        <input type="hidden" name="customTableName" value="' . $customTableName . '">
                        <input type="hidden" name="customRowId" value="' . $idOrAll . '">
                        <button type="submit" class="btn btn-danger">Delete row with ID ' . $idOrAll . ' from table ' . $customTableName . '</button>
                    </form>
                ';
        }
        ?>
